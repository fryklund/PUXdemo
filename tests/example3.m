function [errorL2,errorMax] = example3(varargin)
%% The Poisson equation - Mutiply connected
% Solves the Poisson equation on a mutiply connected domain. See Example 3
...
% input.
PUXstruct.params = struct('ep',2,'coarsen',1,'R_ratio',2.5,'R_p',0.12,'regularity',params.regularity);
...
curve(1) = diffCurve(curves.example3());
curve(2) = diffCurve(curves.example3Exterior());
...
% idx_xeInOmega_boolean is a boolean vector identifying points from xe as
% in Omega (1) or outside (0).

% in_domain_sort uses a BIM with a unit vector to identify points, see REF.
